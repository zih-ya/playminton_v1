...
const EventPage = () => {
  ...
  const [isJoinOrCanceling, setIsJoinOrCanceling] = useState(false);
  ...
  return (
    <div>
      ...
      >
        <Button type="link" onClick={() => navigate("/pastevents")}>
          Events in the past
        </Button>
        <Space direction="vertical">
          <Select
            ...
            onChange={(value) => {
              ...
              if (value === "By Date") {
                setSearchValue(
                  new Date().toISOString().split`-`.join`/`.slice(0, 10)
                );
      ...
      <Wrapper>
        {isLoading ? (
          <Text style={{ marginLeft: 45 }}>Loading...</Text>
        ) : (
          <>
            {datas.length === 0 ? (
                    ...
                    >
                      ...
                      {isLogin ? (
                        <Space>
                          ...
                          <Tooltip placement="bottom" title='Loading'>
